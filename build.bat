...
set "DEPOT_TOOLS_DIR=%CD%\src\chromium.googlesource.com\chromium\tools\depot_tools"
...
set "FLUTTER_DIR=%CD%\src\github.com\flutter\flutter"
...
mkdir %CD%\Downloads 2>nul
set "PATCHFILE=%CD%\Downloads\flutter-PR%%p.patch"
...
:: call powershell -Command "Compress-Archive -Force -DestinationPath '%CD%\Downloads\flutter-%FLUTTER_VERSION%-engine-windows-automationid-x64.zip' -Path '%FLUTTER_DIR%\engine\src\out'"
call "C:\Program Files\7-Zip\7z.exe" a -Tzip "%CD%\Downloads\flutter-%FLUTTER_VERSION%-engine-PR%FLUTTER_PR%-x64.zip" "%FLUTTER_DIR%\engine\src\out" || exit /b 1
